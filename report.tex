...
\title{Efficient Transformer Inference on Commodity Hardware: Optimizing Attention and KV-Cache Under 4\,GB VRAM Constraints}
...
The rapid advancement of Large Language Models (LLMs) has established transformer-based architectures as the dominant paradigm for natural language processing. Models such as Llama-2~\cite{touvron2023llama}, Qwen2.5~\cite{qwen2025}, and Phi-3~\cite{abdin2024phi} demonstrate remarkable capabilities in reasoning, code generation, and multimodal understanding. However, their deployment on consumer-grade hardware remains fundamentally constrained by GPU memory limitations.
...
\begin{enumerate}
    ...
    \item Quantitative analysis of 4-bit and 2-bit KV-cache quantization, demonstrating 13.2\% and 15.1\% reductions in per-token memory at the cost of 18--26\% latency overhead.
...
During autoregressive generation, key and value tensors for all previous tokens must be retained in memory. For a model with $L$ layers, $H$ attention heads, and head dimension $d$, the KV-cache for a sequence of length $S$ requires:
\begin{equation}
M_{\text{KV}} = 2 \times L \times H \times d \times S \times b_{\text{KV}}
\label{eq:kv_mem}
\end{equation}
...
Table~\ref{tab:profiler} presents the GPU kernel-level profiling results for the baseline (eager attention, FP16 KV-cache) configuration. Attention operations constitute the dominant compute bottleneck, consuming 23.0\% of total CUDA time, while 4-bit dequantization accounts for 8.5\%.
...
\caption{Baseline latency scaling with sequence length ($L$). Both p50 and p95 latencies increase linearly from 6{,}201\,ms at $L$=128 to 6{,}649\,ms at $L$=1{,}024.}
...
\caption{Baseline peak VRAM vs. sequence length. VRAM grows from 2{,}034\,MB at $L$=128 to 2{,}160\,MB at $L$=1{,}024, with OOM occurring at $T_{\max}$=7{,}040 tokens.}
...
Table~\ref{tab:attention_full} reports the full attention backend comparison across three prompt lengths. FlashAttention-2 consistently delivers the lowest latency and VRAM footprint across all sequence lengths tested.
...
Table~\ref{tab:correctness} reports the token-level agreement rates between each attention backend and the baseline eager configuration. SDPA variants achieve perfect agreement, while FA2 shows 87\% agreement due to numerical differences in the tiled computation, though the top-1 predicted tokens remain semantically equivalent.
...
To ensure that aggressive quantization does not compromise reasoning ability, we evaluated the model under full compression (NF4 weights + INT4 KV-cache) on three standard NLP benchmarks.
...
\node[decision, below right=0.6cm and -0.3cm of fa2] (long) {Long context\\($>$4K tokens)?};
\node[block, below left=0.6cm and -0.5cm of long] (fa2only) {Use FA2 only\\$T_{\max}$=8{,}064};
\node[block, below right=0.6cm and -0.5cm of long] (combined) {Use FA2 + INT4 KV\\$T_{\max}$=8{,}064};
...
This study demonstrates that the layered combination of 4-bit NF4 weight quantization, FlashAttention-2, and INT4 KV-cache quantization transforms commodity 4\,GB GPUs into viable LLM edge devices. FlashAttention-2 provides the most impactful single optimization, extending maximum context length by 18.9\%. The combined FA2~+ INT4 KV configuration achieves the lowest peak VRAM (2{,}090\,MB at $L$=1{,}024) while maintaining the highest context ceiling (8{,}064 tokens). Zero-shot evaluations confirm that this aggressive compression stack preserves model reasoning capabilities. These results provide actionable deployment guidelines for the growing ecosystem of VRAM-constrained edge inference.
...
\bibitem{qwen2025} J. Bai, S. Bai, Y. Chu, et al., ``Qwen Technical Report,'' \textit{arXiv preprint arXiv:2309.16609}, 2023.
